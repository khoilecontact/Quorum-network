...
async function main(){
  ...
  createContract(quorum.member1, quorum.member1.privateKey, tessera.member1.publicKey, tessera.member2.publicKey)
  .then( async function(privateTxReceipt){
    ...
    console.log(`Use the smart contracts 'set' function to update that value to ${newValue} .. - from member1 to member2`);
    await setValueAtAddress(quorum.member1, privateTxReceipt.contractAddress, newValue, contractAbi, quorum.member1.privateKey, tessera.member1.publicKey, tessera.member2.publicKey);
